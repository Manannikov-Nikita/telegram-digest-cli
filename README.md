...
Если виртуальное окружение не активировано, используйте его Python напрямую:
...
большом контексте API предложит уменьшить `--days` или число источников. Если в
выбранном Python отсутствует библиотека, CLI назовёт пакет и покажет команду
установки зависимостей именно для этого интерпретатора.

Для диагностики можно повторить запуск с полным Python traceback:

```bash
python main.py --days 7 --debug
```

Без `--debug` детали исключения не печатаются. Debug-вывод может содержать
локальные пути и текст ответа API; проверьте и при необходимости скройте их перед
публикацией. В MVP нет планировщика, веб-интерфейса, хранения данных на сервере,
автоматической публикации и поддержки приватных каналов/групп.
