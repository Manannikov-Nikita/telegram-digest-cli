...
В примере выбран Python 3.11. Если установлена более новая версия, замените
`-3.11` обеих команд на её номер, например `-3.12` или `-3.13`.
...
Telegram не вернул ни одного поста, модель не вызывается. Ошибки Telegram или
OpenAI до этапа сохранения не создают новый файл.
...
Debug-вывод, включая сообщения исключений, может содержать локальные пути, имена
источников и текст ответа API. Проверьте и при необходимости скройте эти данные
перед публикацией.
